...
- `NOW.md` is the first file to read when asking: **"What are we finishing next?"**
- `CURRENT.md` records the broader accepted state when asking: **"Where are we at?"**
...
- **"What are we doing now?"** → read `NOW.md` and summarize the active closure targets.
