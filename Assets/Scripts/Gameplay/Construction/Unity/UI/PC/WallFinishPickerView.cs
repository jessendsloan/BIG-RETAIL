...
{
    /// <summary>
    /// Immutable presentation data for one wall-finish catalog card.
    /// </summary>
    public sealed class WallFinishPickerItem
    {
        public WallFinishPickerItem(
            string finishId,
            string tooltip,
            Sprite icon)
        {
            if (string.IsNullOrWhiteSpace(finishId))
            {
                throw new ArgumentException(
                    "A wall-finish picker item requires a finish identifier.",
                    nameof(finishId));
            }
    ...
    /// <summary>
    /// Presentation-only wrapper around the wall-finish catalog elements in the
    /// PC construction toolbar document.
    /// </summary>
    public sealed class WallFinishPickerView : IDisposable
    {
        public const string PanelName =
            "wall-finish-picker";

        public const string ItemsContainerName =
            "wall-finish-picker-items";

        public const string SelectedClassName =
            "is-selected";

        private const string ButtonClassName =
            "wall-finish-picker__button";

        private const string IconClassName =
            "wall-finish-picker__icon";
        ...
        public void SetItems(
            IReadOnlyList<WallFinishPickerItem> items)
        {
            if (items == null)
            {
                throw new ArgumentNullException(
            ...
            for (int index = 0;
                 ...
                 index++)
            {
                WallFinishPickerItem item =
                    items[index]
                    ?? throw new InvalidOperationException(
                        $"Wall-finish picker item {index} is null.");
            ...
            WallFinishPickerItem item)
        {
            Button button =
                ...
                {
                    name =
                        $"wall-finish-{item.FinishId}-button",
                    tooltip =
                        item.Tooltip
                };
            ...
            string elementName)
        {
            ...
            throw new InvalidOperationException(
                $"Wall-finish picker is missing required element '{elementName}'.");
        }
